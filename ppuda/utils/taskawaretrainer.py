    ...
    def update(self, models, task: Task, ghn=None, graphs=None):
        ...
        with torch.autocast(device_type='cuda' if torch.cuda.is_available() else 'cpu',
            ...
            for model in models:
                ...
                if weighted_loss.item() > 5.0:
                    if weighted_loss.item() > 20.0:
                        # Extreme case - strong clipping
                        print(f"WARNING: Extreme loss detected: {weighted_loss.item():.4f} - Hard clamping")
                        weighted_loss = torch.tensor(5.0, device=weighted_loss.device)
                    else:
                        # High but not extreme - soft clipping
                        print(f"WARNING: High loss detected: {weighted_loss.item():.4f} - Soft clamping")
                        # Logarithmic scaling for high values
                        weighted_loss = 5.0 + torch.log(1 + (weighted_loss - 5.0) / 5.0) * 2.5
                #print(f"Weighted loss components: support_weight={self.support_weight} * {support_loss.item():.4f} + query_weight={self.query_weight} * {query_loss.item():.4f} = {weighted_loss.item():.4f}")
                ...
                #print(f"After addition: new total_loss={total_loss.item() if isinstance(total_loss, torch.Tensor) else total_loss:.4f}")
